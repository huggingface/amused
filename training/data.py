...
def add_orig_size_imagenet(example):
    example["orig_size"] = (example["image"].width, example["image"].height)
    return example
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if return_text:
            ...
            processing_pipeline = [
                wds.rename(image="jpg;png;jpeg;webp", input_ids="cls", handler=wds.warn_and_continue),
                wds.map(filter_keys(set(["image", "input_ids"]))),
                wds.map(add_orig_size_imagenet, handler=wds.ignore_and_continue),
                wds.map(partial(image_transform, resolution=resolution), handler=wds.warn_and_continue),
                wds.map_dict(input_ids=tokenize),
            ]
        else:
            processing_pipeline = [
...
class WebdatasetSelect:
    ...
    def __call__(self, x):
        ...
        is_synthetic = "is_synthetic" in x_json and x_json["is_synthetic"]
        if is_synthetic:
            return True
        ...
        # watermark
        if self.max_pwatermark is not None:
            if (
...
def sdxl_synthetic_dataset_map(sample):
    image_keys = ["jpg", "png", "jpeg", "webp"]
    does_image_key_exist = any([image_key in sample for image_key in image_keys])

    # hacky logic: not sdxl synthetic dataset
    if does_image_key_exist:
        return sample
...
def synthetic_dataset_map(sample):
    # assume sdxl or midjourney synthetic dataset
    if "json" not in sample:
        sample["json"] = json.dumps(
            {"aesthetic": 6.5,  "pwatermark": 0.0, "stability_metadata": {"p_nsfwdf": 0.0}, "is_synthetic": True}
        ).encode()
    else:
        json_dict = json.loads(sample["json"])
        # JourneyDB synthetic dataset
        if "pwatermark" not in json_dict:
            json_dict["is_synthetic"] = True
        sample["json"] = json.dumps(json_dict).encode()
    return sample


def add_original_size(sample):
    json = sample["orig_size"]
    json["original_width"] = sample["image"].width
    json["original_height"] = sample["image"].height
    return sample
...
class Text2ImageDataset:
    def __init__(
        ...
        if ".yaml" in train_shards_path_or_url:
            with open(train_shards_path_or_url) as f:
                train_shards_path_or_url = yaml.safe_load(f)
        ...
        if not is_pre_encoded:
            processing_pipeline = [
                ...
                wds.map(add_original_size, handler=wds.warn_and_continue),
                wds.map(partial(image_transform, resolution=resolution), handler=wds.warn_and_continue),
                wds.map_dict(
                    input_ids=tokenize,
        ...
        pipeline = [
            ...
            wds.map(synthetic_dataset_map, handler=wds.ignore_and_continue),
            *([select] if select is not None else []),
            wds.map(sdxl_synthetic_dataset_map, handler=wds.ignore_and_continue),
            wds.shuffle(shuffle_buffer_size),
            *processing_pipeline,
            wds.batched(per_gpu_batch_size, partial=False, collation_fn=default_collate),
        ]
